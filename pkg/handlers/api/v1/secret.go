...
import (
	...
	"github.com/gitlayzer/kuberunner/pkg/types/api/corev1/type_secret"
...
func (s *secret) GetSecrets(ctx *gin.Context) {
	...
	secretResp, err := type_secret.Secret.GetSecrets(client, params.FilterName, params.Namespace, params.Limit, params.Page)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"message": err.Error(),
			"data":    nil,
...
func (s *secret) GetSecretDetail(ctx *gin.Context) {
	...
	secretResp, err := type_secret.Secret.GetSecretDetail(client, params.SecretName, params.Namespace)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"message": err.Error(),
			"data":    nil,
...
func (s *secret) UpdateSecret(ctx *gin.Context) {
	...
	if err := type_secret.Secret.UpdateSecret(client, params.Namespace, params.Content); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"message": err.Error(),
			"data":    nil,
		})
...
func (s *secret) CreateSecret(ctx *gin.Context) {
	var (
		secretCreate = new(type_secret.SecretCreate)
		err          error
	)
	...
	client, err := utils.K8s.GetClient(secretCreate.Cluster)
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"message": err.Error(),
			"data":    nil,
	...
	if err := type_secret.Secret.CreateSecret(client, secretCreate); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"message": err.Error(),
			"data":    nil,
		})
...
func (s *secret) DeleteSecret(ctx *gin.Context) {
	...
	if err := type_secret.Secret.DeleteSecret(client, params.SecretName, params.Namespace); err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{
			"message": err.Error(),
			"data":    nil,
		})
